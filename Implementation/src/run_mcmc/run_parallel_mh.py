...
def run_single_chain(init_state, iterations, param_lower, param_upper, likelihood_dependence, sd_likelihood, sd_transition_factor, max_probability):
    # Define likelihood
...
def run_mcmc_mh_parallel(num_chains=4, sd_transition_factor=6, likelihood_dependence=False, sd_likelihood=1, max_probability=False, iterations=2500):
    ...
    with mp.Pool(num_chains) as pool:
        results = pool.starmap(run_single_chain, [(state, iterations, param_lower, param_upper, likelihood_dependence, sd_likelihood, sd_transition_factor, max_probability) for state in init_states])
