...
if __name__ == "__main__": 
    ...
    for item in range(len(to_benchmark)):
        ...
        for case in test_case:
            ...
            sampled_params, _ = run_mcmc(niterations=niterations, \
                                        nchains=nchains, \
            ...
            samples = pd.DataFrame(combined_results, columns=['TT','C0','beta','ETF','FC','FRAC','K2'])
            ...
            param_vec = []
            for i in range(7):
                param_vec.append(np.random.choice(samples.iloc[:, i], monte_carlo_number))
