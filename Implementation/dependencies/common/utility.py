...
class UQOutputPaths(object):
    ...
    def _update_other_paths_based_on_workingDir(self, workingDir):
        ...
        self.master_configuration_folder = (
            self.model_runs_folder / "master_configuration"
        )
        ...
        self.df_measured_file = (
            self.model_runs_folder / "df_measured.pkl"
        )  # model_runs_folder/"df_measured.pkl"
        self.df_past_simulated_file = (
            self.model_runs_folder / "df_past_simulated.pkl"
        )  # "df_simulated.pkl"
        self.df_unaltered_file = (
            self.model_runs_folder / "df_unaltered.pkl"
        )  # "df_unaltered_ergebnis.pkl"
        ...
        self.df_all_index_parameter_gof_file = (
            self.model_runs_folder / "df_all_index_parameter_gof_values.pkl"
        )
        self.df_all_index_parameter_file = (
            self.model_runs_folder / "df_all_index_parameter_values.pkl"
        )

        # Files produced by UQEF.Statistics and LarsimStatistics
        self.statistics_dictionary_file = (
            self.model_runs_folder / "statistics_dictionary_qoi_Value.pkl"
        )
        ...
        self.dict_of_approx_matrix_c_file = (
            self.model_runs_folder / "dict_of_approx_matrix_c.pkl"
        )
        self.dict_of_matrix_c_eigen_decomposition_file = (
            self.model_runs_folder / "dict_of_matrix_c_eigen_decomposition.pkl"
        )
    ...
    def update_specifi_model_run_output_file_paths(self, model_runs_folder, i):
        ...
        """
        parameters_Larsim_run_file = (
            model_runs_folder / f"parameters_Larsim_run_{i}.pkl"
        )
        parameters_Larsim_run_failed_file = (
            model_runs_folder / f"parameters_Larsim_run_{i}_failed.pkl"
        )
        ...
        return (
            ...
            df_Larsim_run_file,
            gof_file,
            gradients_matrices_file,
            df_Larsim_raw_run_fileh,
        )


def get_df_from_simulationNodes(
    simulationNodes, nodes_or_paramters="nodes", params_list=None
):
    ...
    if nodes_or_paramters == "nodes":
        if params_list is None:
            ...
            my_ditc = {
                f"{params_list[i]}": simulationNodes.nodes[i, :] for i in range(numDim)
            }
    else:
        if params_list is None:
            ...
            # important assumption about the order of the nodes and their corresponding names
            my_ditc = {
                f"{params_list[i]}": simulationNodes.parameters[i, :]
...
def generate_df_with_nodes_and_weights_from_file(file_path, params_list=None):
    ...
    if params_list is None:
        ...
        # important assumption about the order of the nodes and their corresponding names
        my_ditc = {
            f"{params_list[i]}": nodes_and_weights_array[:, i] for i in range(numDim)
        }
...
def plot_2d_matrix_static_from_list(simulationNodes_list, title="Plot nodes"):
    ...
    g = sns.pairplot(
        dfsimulationNodes, vars=list(dfsimulationNodes.columns), corner=True
    )
...
def plot_3d_dynamic(dfsimulationNodes):
    ...
    fig = px.scatter_3d(
        dfsimulationNodes, x=my_columns[0], y=my_columns[1], z=my_columns[2]
    )
    return fig
...
def plot_3d_statis(simulationNodes, nodes_or_paramters="nodes"):
    ...
    if nodes_or_paramters == "nodes":
        axs.scatter(
            list(simulationNodes.nodes[0, :]),
            list(simulationNodes.nodes[1, :]),
            list(simulationNodes.nodes[2, :]),
            marker="o",
        )
        plt.title("Nodes\n")
    else:
        axs.scatter(
            list(simulationNodes.parameters[0, :]),
            list(simulationNodes.parameters[1, :]),
            list(simulationNodes.parameters[2, :]),
            marker="o",
        )
...
def generate_table_single_rule_over_dim_and_orders_sparse_and_nonsparse(
    rule, dists, dim, q_orders, growth=None
):
    ...
    for dist_i, dist in enumerate(dists):
        for q_i, order in enumerate(q_orders):
            abscissas, weights = cp.generate_quadrature(
                order, dist, rule=rule, sparse=True, growth=growth
            )
            num_nodes[dist_i][q_i][0] = len(abscissas.T)
            abscissas, weights = cp.generate_quadrature(
                order, dist, rule=rule, sparse=False, growth=growth
            )
        ...
        for q_i, order in enumerate(q_orders):
            table.append(
                [
                    ...
                    dim[dist_i],
                    order,
                    num_nodes[dist_i][q_i][0],
                    num_nodes[dist_i][q_i][1],
                ]
            )
    print(
        tabulate(
            table,
            headers=["rule", "dim", "q", "#nodes sparse tensor", "#nodes full tensor"],
            numalign="right",
        )
    )


def generate_table_single_rule_over_dim_and_orders(
    rule, dists, dim, q_orders, sparse=True, growth=None
):
    ...
    for dist_i, dist in enumerate(dists):
        for q_i, order in enumerate(q_orders):
            abscissas, weights = cp.generate_quadrature(
                order, dist, rule=rule, sparse=sparse, growth=growth
            )
            num_nodes[dist_i][q_i][0] = len(abscissas.T)
    ...
    print(
        tabulate(
            table, headers=["rule", "dim", "q", table_column_name], numalign="right"
        )
    )
...
def plot_2d_matrix_of_nodes_over_orders(rule, dist, orders, sparse=False, growth=None):
    ...
    for order in orders:
        abscissas, weights = cp.generate_quadrature(
            order, dist, rule=rule, growth=growth, sparse=sparse
        )
        ...
        sns.set(style="ticks", color_codes=True)
        g = sns.pairplot(
            df_nodes_weights, vars=list(dict_for_plotting.keys()), corner=True
        )
        if growth:
...
def generate_table_over_rules_orders_for_single_dim(
    rules, dist, dim, q_orders, growth=None
):
    ...
    print(
        tabulate(
            ...
            headers=[
                ...
                "#nodes full tensor",
                "#nodes sparse",
                "#nodes sparse < #nodes full tensor",
            ],
            numalign="right",
        )
    )
...
def transform_column_in_df(
    df, transformation_function_str, column_name, new_column_name=None
):
    if new_column_name is None:
        new_column_name = column_name
    if transformation_function_str == "log":
        df[column_name] = df[column_name].apply(lambda x: x if x > 1e-4 else 1e-4)
        df[new_column_name] = np.log(
            df[column_name],
            out=np.zeros_like(df[column_name].values),
            where=(df[column_name].values != 0),
        )
    elif transformation_function_str == "log2":
        df[column_name] = df[column_name].apply(lambda x: x if x > 1e-4 else 1e-4)
        df[new_column_name] = np.log2(
            df[column_name],
            out=np.zeros_like(df[column_name].values),
            where=(df[column_name].values != 0),
        )
...
_all_functions = [
    objectivefunctions.MAE,
    objectivefunctions.MSE,
    objectivefunctions.RMSE,
    objectivefunctions.NRMSE,
    objectivefunctions.RSR,
    objectivefunctions.BIAS,
    objectivefunctions.PBIAS,
    objectivefunctions.ROCE,
    objectivefunctions.NSE,
    objectivefunctions.LogNSE,
    objectivefunctions.LogGaussian,
    objectivefunctions.CorrelationCoefficient,
    ...
    if gof_list == "all" or gof_list is None:
        ...
        for idx, f in enumerate(gof_list):
            if not callable(f) and f in mapping_gof_names_to_functions:
                ...
                raise ValueError(
                    "Not proper specification of Goodness of Fit function name"
                )
    return gof_list


def _check_if_measured_or_predicted_are_empty(
    measuredDF, predictedDF, measuredDF_column_name, simulatedDF_column_name, gof_list
):
    if (
        measuredDF.empty
        or predictedDF.empty
...
def filter_two_DF_on_common_timesteps(
    DF1, DF2, saveToFile=None, column_name_df1="TimeStamp", column_name_df2=None
):
    """
...
def calculateGoodnessofFit_simple(
    measuredDF,
    simulatedDF,
    gof_list,
    ...
    **kwargs,
    ...
    is_empty, result_dict = _check_if_measured_or_predicted_are_empty(
        measuredDF,
        simulatedDF,
        measuredDF_column_name,
        simulatedDF_column_name,
        gof_list,
    )
    if is_empty:
        return result_dict
...
def check_if_file_exists_pathlib(file_path, message=None):
    if not _check_is_file_pathlib(file_path) or not _check_if_file_exists_pathlib(
        file_path
    ):
...
def pathlib_to_from_str(path, transfrom_to="str"):
    ...
    if transfrom_to == "Path" and isinstance(path, str):
        ...
        return str(path)
    elif (transfrom_to == "str" and isinstance(path, str)) or (
        transfrom_to == "Path" and isinstance(path, pathlib.PosixPath)
    ):
        return path
    else:
        raise ValueError(
            f"Error in pathlib_to_from_str fucnction got"
...
def update_output_file_paths_based_on_workingDir(workingDir):
    ...
    df_all_index_parameter_gof_file = (
        workingDir / "df_all_index_parameter_gof_values.pkl"
    )
    df_all_index_parameter_file = workingDir / "df_all_index_parameter_values.pkl"
    df_time_varying_grad_analysis_file = (
        workingDir / "df_time_varying_grad_analysis.pkl"
    )
    df_time_aggregated_grad_analysis_file = (
        workingDir / "df_time_aggregated_grad_analysis.pkl"
    )
    ...
    dict_of_matrix_c_eigen_decomposition_file = (
        workingDir / "dict_of_matrix_c_eigen_decomposition.pkl"
    )
    ...
    return (
        ...
        parameters_file,
        args_file,
        configuration_object_file,
        df_all_simulations_file,
        df_all_index_parameter_gof_file,
        df_all_index_parameter_file,
        df_time_varying_grad_analysis_file,
        df_time_aggregated_grad_analysis_file,
        statistics_dictionary_file,
...
def parse_datetime_configuration(time_settings_config):
    ...
    elif isinstance(time_settings_config, list) or isinstance(
        time_settings_config, tuple
    ):
        (
            start_year,
            start_month,
            ...
            end_hour,
            end_minute,
        ) = time_settings_config
    else:
        raise Exception(
            f"[Error] in parse_datetime_configuration: time_settings_config should be "
...
def return_configuration_object(configurationObject):
    if isinstance(configurationObject, str) or isinstance(
        configurationObject, pathlib.PosixPath
    ):
        try:
...
def check_if_configurationObject_is_in_right_format_and_return(
    configurationObject: Union[dict, pd.DataFrame, str, pathlib.PosixPath, Any],
    raise_error: Optional[bool] = True,
):
    if isinstance(configurationObject, dict) or isinstance(
        configurationObject, pd.DataFrame
    ):
        return configurationObject
    elif isinstance(configurationObject, str) or isinstance(
        configurationObject, pathlib.PosixPath
    ):
        configurationObject = read_configuration_dict_from_json_file(
            configurationObject
        )
        return configurationObject
    else:
        if raise_error:
            raise Exception(
                "Error in configure_parameters_value function - configurationObject is not in the expected form!"
            )
        else:
            return None
...
def get_configuration_value(
    key: str, config_dict: dict, default_value: Any, **kwargs
) -> Any:
    """
...
def get_simulation_settings(
    configurationObject, key_from_configurationObject="simulation_settings"
):
    """
...
def handle_multiple_qoi(qoi, qoi_column, result_dict):
    ...
    multiple_qoi = False
    number_of_qois = 1
    if (isinstance(qoi, list) and isinstance(qoi_column, list)) or (
        qoi == "GoF" and isinstance(qoi_column, list)
    ):
        multiple_qoi = True
...
def handle_transform_model_output(
    transform_model_output, result_dict, dict_config_simulation_settings
):
    """
...
def read_simulation_settings_from_configuration_object_refactored(
    configurationObject: dict, **kwargs
) -> dict:
    ...
    transform_model_output = get_configuration_value(
        "transform_model_output", dict_config_simulation_settings, "None", **kwargs
    )
    transform_model_output = handle_transform_model_output(
        transform_model_output, result_dict, dict_config_simulation_settings
    )
...
def read_simulation_settings_from_configuration_object(
    configurationObject: dict, **kwargs
) -> dict:
    """
    ...
    dict_config_simulation_settings = configurationObject.get(
    ...
    result_dict["autoregressive_model_first_order"] = strtobool(
        dict_config_simulation_settings.get("autoregressive_model_first_order", "False")
    )
    ...
    if "transform_model_output" in kwargs:
        ...
        if result_dict["multiple_qoi"]:
            try:
                transform_model_output = dict_config_simulation_settings[
                    "transform_model_output"
                ]
                for idx, single_transform_model_output in enumerate(
                    transform_model_output
                ):
                    if single_transform_model_output == "None":
                        transform_model_output[idx] = None
            ...
            transform_model_output = dict_config_simulation_settings.get(
                "transform_model_output", "None"
            )
            if transform_model_output == "None":
                transform_model_output = None
    ...
    if "read_measured_data" in kwargs:
        ...
        if result_dict["multiple_qoi"]:
            ...
            read_measured_data = dict_config_simulation_settings.get(
    ...
    if "qoi_column_measured" in kwargs:
        ...
        if result_dict["multiple_qoi"]:
            try:
                qoi_column_measured = dict_config_simulation_settings[
                    "qoi_column_measured"
                ]
            ...
            qoi_column_measured = dict_config_simulation_settings.get(
    ...
    calculate_GoF = strtobool(
        dict_config_simulation_settings.get("calculate_GoF", "False")
    )
    ...
    if result_dict["qoi"] == "GoF":
        ...
        if result_dict["multiple_qoi"]:
            ...
            if not read_measured_data:
                raise ValueError
        objective_function_qoi = dict_config_simulation_settings.get(
        ...
        if isinstance(objective_function_qoi, list):
            ...
            list_objective_function_qoi = [
                objective_function_qoi,
            ]
    ...
    for idx, single_qoi_column in enumerate(list_qoi_column):
        dict_qoi_column_and_measured_info[single_qoi_column] = (
            list_read_measured_data[idx],
            list_qoi_column_measured[idx],
            list_transform_model_output[idx],
        )
    ...
    if center != "center" and center != "left" and center != "right":
        raise Exception(
            f'[ERROR:] center should be either "center" or "left" or "right"'
        )
    ...
    compute_gradients = strtobool(
        dict_config_simulation_settings.get("compute_gradients", "False")
    )
    ...
    if compute_gradients:
        gradient_method = dict_config_simulation_settings.get(
        ...
        if gradient_method == "Central Difference":
            ...
            raise Exception(
                f"NUMERICAL GRADIENT EVALUATION ERROR: "
...
def get_list_of_parameters_dicts_from_configuration_dict(
    configurationObject: Union[dict, List, Any], raise_error: Optional[bool] = False
) -> List[dict]:
    configurationObject = check_if_configurationObject_is_in_right_format_and_return(
        configurationObject, raise_error=raise_error
    )
    result_list = []
...
def get_param_info_dict_from_configurationObject(
    configurationObject: Union[dict, List, Any]
) -> Dict[str, dict]:
    """
    ...
    if isinstance(configurationObject, dict):
        list_of_parameters = get_list_of_parameters_dicts_from_configuration_dict(
...
def get_param_info_dict(
    default_par_info_dict: dict, configurationObject: Union[dict, List, Any] = None
) -> Dict[str, dict]:
    """
...
def configuring_parameter_values(
    parameters,
    configurationObject: Union[dict, List],
    default_par_info_dict,
    take_direct_value: bool = False,
    ...
    if isinstance(parameters, dict) and take_direct_value:
        ...
        for single_param in list_of_parameters:
            if single_param["distribution"] != "None":
                ...
                parameters_dict[single_param["name"]] = parameters[
                    uncertain_param_counter
                ]
                ...
                if take_direct_value:
                    ...
                    if isinstance(default_par_info_dict[single_param["name"]], dict):
                        parameters_dict[single_param["name"]] = default_par_info_dict[
                            single_param["name"]
                        ]["default"]
                    else:
                        parameters_dict[single_param["name"]] = default_par_info_dict[
...
def update_parameter_dict_for_gradient_computation(
    parameters,
    configurationObject,
    take_direct_value=False,
    perturb_single_param_around_nominal=False,
    parameter_index_to_perturb=0,
    eps_val=1e-4,
):
    ...
    parameter_lower_limit = (
        param_entry_dict["lower_limit"] if "lower_limit" in param_entry_dict else None
    )
    parameter_upper_limit = (
        param_entry_dict["upper_limit"] if "upper_limit" in param_entry_dict else None
    )
    param_h = eps_val * (parameter_upper_limit - parameter_lower_limit)
    parameter_lower_limit += param_h
    parameter_upper_limit -= param_h
...
def _get_df_simulation_from_file(
    working_folder, df_all_simulations_name="df_all_simulations.pkl"
):
...
def _get_df_index_parameter_gof_from_file(
    working_folder,
    df_all_index_parameter_gof_values_file_name="df_all_index_parameter_gof_values.pkl",
):
    df_all_index_parameter_gof_values_file = (
        pathlib.Path(working_folder) / df_all_index_parameter_gof_values_file_name
    )
    df_all_index_parameter_gof_values = pd.read_pickle(
        df_all_index_parameter_gof_values_file, compression="gzip"
    )
    return df_all_index_parameter_gof_values


def _get_statistics_dict(
    working_folder, statistics_dictionary_file_name="statistics_dictionary.pkl"
):
    statistics_dictionary_file = (
        pathlib.Path(working_folder) / statistics_dictionary_file_name
    )
...
def create_statistics_dictionary_from_saved_single_qoi_statistics_dictionary(
    workingDir, list_qoi_column
):
    statistics_dictionary = defaultdict(dict)
    for single_qoi in list_qoi_column:
        statistics_dictionary_file_temp = (
            workingDir / f"statistics_dictionary_qoi_{single_qoi}.pkl"
        )
...
def _get_parameter_columns_df_index_parameter_gof(df_index_parameter_gof):
    def check_if_column_stores_parameter(x):
        if isinstance(x, tuple):
            x = x[0]
        return (
            x not in list(mapping_gof_names_to_functions.keys())
            and not x.startswith("d_")
            and x not in ["index_run", "station", "successful_run", "qoi"]
        )

    return [
        x
        for x in df_index_parameter_gof.columns.tolist()
        if check_if_column_stores_parameter(x)
    ]
...
def _get_gof_columns_df_index_parameter_gof(df_index_parameter_gof):
    def check_if_column_stores_gof(x):
        if isinstance(x, tuple):
            x = x[0]
        return x in list(mapping_gof_names_to_functions.keys()) and x not in [
            "index_run",
            "station",
            "successful_run",
            "qoi",
    ...
    return [
        x
        for x in df_index_parameter_gof.columns.tolist()
        if check_if_column_stores_gof(x)
    ]
...
def plot_subplot_params_hist_from_df(df_index_parameter_gof):
    columns_with_parameters = _get_parameter_columns_df_index_parameter_gof(
...
def plot_subplot_params_hist_from_df_conditioned(
    df_index_parameter_gof,
    name_of_gof_column="NSE",
    threshold_gof_value=0,
    comparison="smaller",
):
    """
...
def plot_scatter_matrix_params_vs_gof(
    df_index_parameter_gof, name_of_gof_column="NSE", hover_name="index_run"
):
    columns_with_parameters = _get_parameter_columns_df_index_parameter_gof(
        df_index_parameter_gof
    )
...
def plot_surface_2d_params_vs_gof(
    df_index_parameter_gof,
    param1,
    param2,
    num_of_points_in_1d=8,
    name_of_gof_column="NSE",
):
    ...
    fig.update_layout(
        scene=dict(
            xaxis_title=param1, yaxis_title=param2, zaxis_title=name_of_gof_column
        ),
        autosize=False,
        title="GoF Plot",
...
def plot_parallel_params_vs_gof(
    df_index_parameter_gof, name_of_gof_column="NSE", list_of_params=None
):
    if list_of_params is None:
        list_of_params = _get_parameter_columns_df_index_parameter_gof(
            df_index_parameter_gof
        )
    dimensions = list_of_params + [
        name_of_gof_column,
    ]
    fig = px.parallel_coordinates(
        df_index_parameter_gof, color=name_of_gof_column, dimensions=dimensions
    )
    return fig


def plot_scatter_matrix_params_vs_gof_seaborn(
...
def scatter_3d_params_static(df_index_parameter_gof, param1, param2, param3):
    ...
    axs = fig.gca(projection="3d")
    axs.scatter(
        list(df_index_parameter_gof[param1]),
        list(df_index_parameter_gof[param2]),
        list(df_index_parameter_gof[param3]),
        marker="o",
    )
    plt.title("Parameters\n")
...
def running_the_model_over_time_and_parameters(
    model,
...
def generate_quadrature_nodes_and_weights(
    joint_dist,
...
def generating_parameters_from_nodes(nodes_quad, joint_dist_standard, joint_dist):
    ...
    """
    parameters_quad = transformation_of_parameters(
        nodes_quad, joint_dist_standard, joint_dist
    )
    return parameters_quad


def generate_polynomial_expansion(
    joint_dist,
    order: int,
    rule: str = "three_terms_recurrence",
    poly_normed: bool = True,
):
    """
...
def compute_gPCE_over_time_(
    ...
    weights_quad: np.ndarray = None,
    poly_rule: str = "three_terms_recurrence",
    poly_normed: bool = True,
):
    """
    ...
    number_expansion_coefficients = int(
        scipy.special.binom(dim + expansion_order, dim)
    )  # cp.dimensions(polynomial_expansion)
    print(
        f"Total number of expansion coefficients in {dim}D space: {int(number_expansion_coefficients)}"
    )

    gPCE_over_time = np.empty(
        (len(t),), dtype=object
    )  # np.empty((len(t), number_expansion_coefficients))
    coeff = np.empty((len(t),), dtype=object)
    for idx, _ in enumerate(model_runs):  # for element in t:
        if regression:
            gPCE_over_time[idx] = cp.fit_regression(
                polynomial_expansion, nodes, model_runs[idx]
            )
        else:
            gPCE_over_time[idx], coeff[idx] = cp.fit_quadrature(
                polynomial_expansion, nodes, weights_quad, model_runs[idx], retall=True
            )
    return gPCE_over_time, polynomial_expansion, np.asfarray(norms), np.asfarray(coeff)


def compute_PSP_over_time_(
    model,
    t: Union[np.array, np.ndarray, List[Union[int, float]]],
    joint_dist_standard,
    joint_dist,
    quadrature_order: int,
    expansion_order: int,
    rule_quadrature: str = "g",
    growth: bool = False,
    sparse: bool = False,
    poly_rule: str = "three_terms_recurrence",
    poly_normed: bool = True,
):
    nodes_quad, weights_quad = generate_quadrature_nodes_and_weights(
        joint_dist_standard, quadrature_order, rule_quadrature, growth, sparse
    )
    parameters_quad = generating_parameters_from_nodes(
        nodes_quad, joint_dist_standard, joint_dist
    )
    polynomial_expansion, norms = generate_polynomial_expansion(
        joint_dist_standard, expansion_order, poly_rule, poly_normed
    )
    ...
    for idx, _ in enumerate(model_runs):  # for element in t:
        gPCE_over_time[idx], coeff[idx] = cp.fit_quadrature(
            polynomial_expansion, nodes_quad, weights_quad, model_runs[idx], retall=True
        )
    return gPCE_over_time, polynomial_expansion, np.asfarray(norms), np.asfarray(coeff)
