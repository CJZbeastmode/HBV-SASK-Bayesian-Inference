...
if __name__ == "__main__": 
    ...
    for test_case in toTest:
        ...
        sampled_params, total_iterations = run_mcmc(sd_transition_factor=test_case)
        end = time.time()
        print(f'Time needed for test case {test_case}: {str(end - start)}')
        ...
        results.append([test_case, rmse_mean, rmse_max, mae_mean, mae_max])

    np.savetxt('sensitivity_transition.txt', results, delimiter=',', header='Test_Case, RMSE_Mean, RMSE_Max, MAE_Mean, MAE_Max', comments='')
